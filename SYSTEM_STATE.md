...
**4) OD-011 — ilk-geçiş (salt-okunur; knowledge.sqlite'a YAZILMADI).** 252 event'in 252'si canonical-v1'e maplenmiş, 167 cycle (deflasyon 0.66). **HOUR17 popülasyonu: 68 event → 53 bağımsız cycle (deflasyon 0.78)** — min-N=20 tabanının üzerinde, ancak gate'li alt-iddiaların (regime) cycle-düzeyi WR/mc recompute'u hâlâ gerekli. **MONDAY-VETO: ami_events'te yalnız 1 Pazartesi anchor'ı — popülasyonu canonical store'da YOK → recompute BLOCKED_BY_SOURCE** (araştırma evrenleri ingest edilmeli). Kalan 6 KO feature-gated — frozen popülasyon manifestleri olmadan cycle eşlemesi yapılamaz; tam recompute ayrı Phase 6 dalgası olarak kalıyor (kapsamı artık net).
