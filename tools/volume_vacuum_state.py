...
def _compute_state(summary: Dict[str, Any]) -> Tuple[str, List[str]]:
    ...
    avg_trade_intensity = _safe_float(summary.get("avg_trade_intensity_tagged"))
    avg_spread = _safe_float(summary.get("avg_spread_tagged"))
    reasons: List[str] = []
    if high_count >= 2:
        reasons.append("multiple_high_volume_vacuum")
    if recent_alert_count >= 8:
        reasons.append("dense_volume_vacuum_cluster")
    if avg_trade_intensity <= 50.0 and avg_spread >= 0.0001:
        reasons.append("extreme_thin_market")
    if reasons:
        return ("severe", reasons)
    if recent_alert_count >= 3:
        reasons.append("recent_volume_vacuum_cluster")
    if tagged_rate >= 0.03:
        reasons.append("elevated_volume_vacuum_rate")
    if avg_trade_intensity <= 150.0:
        reasons.append("low_trade_intensity")
    if reasons:
...
def _recommended_action(level: str, freshness_status: str) -> str:
...
def build_state_payload(
    *,
    ...
    recommended_action = _recommended_action(state, str(freshness["status"]))
    payload = {
        "lane": "volume_vacuum",
        "source_json": str(source_json),
        "symbol": str(alert_payload.get("symbol") or "").upper(),
        "state": {"level": state, "reasons": reasons, "freshness": dict(freshness)},
        "dashboard_summary": (
            f"{str(alert_payload.get('symbol') or '').upper()} {state} volume vacuum, "
            f"{_safe_int(summary.get('recent_alert_count'))} recent alerts, freshness {str(freshness['status'])}."
        ),
        "notification_text": (
            f"[volume-vacuum] symbol={str(alert_payload.get('symbol') or '').upper()} level={state} freshness={str(freshness['status'])} "
            f"recent_alerts={_safe_int(summary.get('recent_alert_count'))} avg_trade_intensity={_safe_float(summary.get('avg_trade_intensity_tagged')):.2f} "
            f"avg_spread={_safe_float(summary.get('avg_spread_tagged')):.6f} action={recommended_action}"
        ),
        "recommended_action": recommended_action,
        "card": {
            "headline": f"{str(alert_payload.get('symbol') or '').upper()} volume vacuum {state}",
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compress volume vacuum alerts into a runtime-friendly state payload.")
    p.add_argument("--alerts-json", default="reports/VOLUME_VACUUM_ALERTS.json")
    p.add_argument("--out-json", default="reports/VOLUME_VACUUM_STATE.json")
    p.add_argument("--out-md", default="reports/VOLUME_VACUUM_STATE.md")
