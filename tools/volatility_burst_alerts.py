...
def _tag_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    trade_intensity = [_safe_float(r.get("trade_intensity")) for r in rows if r.get("trade_intensity") is not None]
    spread = [_safe_float(r.get("spread")) for r in rows if r.get("spread") is not None]
    abs_ret_q90 = _quantile(abs_ret, 0.90)
    abs_ret_q75 = _quantile(abs_ret, 0.75)
    intensity_q60 = _quantile(trade_intensity, 0.60)
    intensity_q40 = _quantile(trade_intensity, 0.40)
    spread_q75 = _quantile(spread, 0.75)
    tagged: List[Dict[str, Any]] = []
    for row in rows:
        ret_1 = _safe_float(row.get("ret_1"))
        abs_move = abs(ret_1)
        intensity = _safe_float(row.get("trade_intensity"))
        row_spread = _safe_float(row.get("spread"))
        direction = "UP" if ret_1 > 0 else "DOWN" if ret_1 < 0 else "FLAT"
        if abs_move >= abs_ret_q90 and intensity >= intensity_q60:
            severity = "high"
            fired = True
        elif abs_move >= abs_ret_q75 and intensity >= intensity_q40 and row_spread <= max(spread_q75, 0.0):
            severity = "medium"
            fired = True
        else:
            severity = "none"
        ...
        tagged.append(
            {
                ...
                "tag": "volatility_burst" if fired else "normal",
                "rule_fired": fired,
                "severity": severity,
                "direction": direction,
                "ret_1": ret_1,
    ...
    *,
    ...
    summary = {
        ...
        "avg_abs_ret_1_tagged": (
            sum(_safe_float(r.get("abs_ret_1")) for r in fired) / float(len(fired)) if fired else 0.0
        ),
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build volatility burst alerts from microstructure DB.")
    ...
    p.add_argument("--out-json", default="reports/VOLATILITY_BURST_ALERTS.json")
    p.add_argument("--out-md", default="reports/VOLATILITY_BURST_ALERTS.md")
