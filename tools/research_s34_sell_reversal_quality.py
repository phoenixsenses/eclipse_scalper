...
OUT_JSON = Path("reports/research/s34/S34_SELL_LIQ_REVERSAL_LONG_2026-06-07_15.json")
OUT_MD = Path("reports/research/s34/S34_SELL_LIQ_REVERSAL_LONG_2026-06-07_15.md")
...
def _horizon_marks(con: sqlite3.Connection, start_ms: int, end_ms: int) -> list[tuple[int, float]]:
    ...
    CONSUMER-MIGRATION-V3). No longer called by `sim_long`; the reader-
...
def main():
    ...
    for threshold in [50_000, 100_000, 200_000, 300_000]:
        for tp_bps in [40, 60, 80, 120]:
            for delay_sec in [0, 60, 120, 300]:
                trades = []
                for day in DAYS:
                    for ts_ms, notional in events(con, day, threshold):
                        trade = sim_long(con, ts_ms, threshold, tp_bps, root=root, source_db_path=SOURCE_DB_PATH,
                                          entry_delay_sec=delay_sec)
                        if trade:
                            trade["notional"] = notional
                            trades.append(trade)
                name = f"SELL_REVERSAL_LONG {threshold} TP{tp_bps} DELAY{delay_sec}s"
                summary = summarize(name, trades)
                rows.append(summary)
                details[name] = trades

    rows = sorted(rows, key=lambda r: r.get("mean_net_bps", -999), reverse=True)
    ...
    lines.extend(
        [
            ...
            "## Verdict",
            "",
            "SELL liquidation reversal LONG does not currently produce a convincing candidate. The top rows should be treated with caution if N is small or driven by delayed entries. This remains research-only unless a later larger sample shows a stable positive median and day spread.",
        ]
    )
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(OUT_MD)
