...
OUT_JSON = ROOT / "reports" / "research" / "s34" / "S34_SOL_200K_ROUTE_SWEEP.json"
OUT_MD = ROOT / "reports" / "research" / "s34" / "S34_SOL_200K_ROUTE_SWEEP.md"
...
def main() -> int:
    ...
    if not max_ts:
        raise RuntimeError("no SOL liquidation rows")
    end_ms = int(max_ts)
    start_ms = end_ms - LOOKBACK_DAYS * 24 * 3600 * 1000

    base_rule = S34Rule(
        name="SOL_BUY_LIQ_LONG_200K_ROUTE_SWEEP",
        symbol=SYMBOL,
        liq_side=LIQ_SIDE,
        direction="LONG",
    ...
    lines.extend(
        [
            ...
            "## Read",
            "",
            f"Best real-fill test median among reported routes: `{best_real[0]}`. This is retrospective over 75 combinations; promotion would require a separate exploratory pre-registration. Existing live SOL rule remains unchanged by this report.",
        ]
    )
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")
    source_con.close()
