...
from tools.volume_vacuum_alerts import build_payload as build_alert_payload
from tools.volume_vacuum_state import build_state_payload
...
def _priority(state_level: str, freshness_status: str, recent_alert_count: int, avg_spread_tagged: float) -> float:
    ...
    if str(freshness_status) != "fresh":
        base -= 75.0
    return base + min(50.0, float(recent_alert_count)) + min(50.0, float(avg_spread_tagged) * 500000.0)
...
def _build_banner(summary: Dict[str, Any], top_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "headline": f"Volume vacuum watchlist top={symbol or 'none'} level={level} freshness={freshness} action={action}",
    ...
    *,
    ...
    for raw_symbol in symbols:
        ...
        alert_payload = build_alert_payload(
            ...
            out_json=f"reports/{symbol}_VOLUME_VACUUM_ALERTS.json",
            out_md=f"reports/{symbol}_VOLUME_VACUUM_ALERTS.md",
        )
        state_payload = build_state_payload(
            alert_payload=alert_payload,
            source_json=f"reports/{symbol}_VOLUME_VACUUM_ALERTS.json",
            out_json=f"reports/{symbol}_VOLUME_VACUUM_STATE.json",
            out_md=f"reports/{symbol}_VOLUME_VACUUM_STATE.md",
        )
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build a multi-symbol volume vacuum watchlist.")
    ...
    p.add_argument("--out-json", default="reports/VOLUME_VACUUM_WATCHLIST.json")
    p.add_argument("--out-md", default="reports/VOLUME_VACUUM_WATCHLIST.md")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    lines = [
        ...
        "",
        "| symbol | state_level | freshness_status | recommended_action | recent_alert_count | high_count | medium_count | avg_trade_intensity_tagged | avg_spread_tagged | priority_score |",
        "|---|---|---|---|---:|---:|---:|---:|---:|---:|",
    ]
    for row in payload["rows"]:
        lines.append(
            ...
            f"{float(row['avg_trade_intensity_tagged']):.2f} | {float(row['avg_spread_tagged']):.6f} | {float(row['priority_score']):.2f} |"
        )
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
