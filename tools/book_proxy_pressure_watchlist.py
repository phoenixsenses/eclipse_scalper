...
from tools.book_proxy_pressure_alerts import build_payload as build_alert_payload
from tools.book_proxy_pressure_state import build_state_payload
...
def _priority(state_level: str, freshness_status: str, recent_alert_count: int, avg_abs_imbalance_tagged: float) -> float:
    base = {"quiet": 0.0, "elevated": 100.0, "severe": 200.0}.get(str(state_level), 0.0)
    if str(freshness_status) != "fresh":
        base -= 75.0
    return base + min(50.0, float(recent_alert_count)) + min(50.0, float(avg_abs_imbalance_tagged) * 100.0)
...
def _build_banner(summary: Dict[str, Any], top_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "headline": f"Book proxy pressure watchlist top={symbol or 'none'} level={level} freshness={freshness} action={action}",
    ...
    *,
    ...
    for raw_symbol in symbols:
        ...
        alert_payload = build_alert_payload(
            ...
            out_json=f"reports/{symbol}_BOOK_PROXY_PRESSURE_ALERTS.json",
            out_md=f"reports/{symbol}_BOOK_PROXY_PRESSURE_ALERTS.md",
        )
        state_payload = build_state_payload(
            alert_payload=alert_payload,
            source_json=f"reports/{symbol}_BOOK_PROXY_PRESSURE_ALERTS.json",
            out_json=f"reports/{symbol}_BOOK_PROXY_PRESSURE_STATE.json",
            out_md=f"reports/{symbol}_BOOK_PROXY_PRESSURE_STATE.md",
        )
        ...
        rows.append(
            {
                ...
                "avg_abs_imbalance_tagged": _safe_float(card.get("avg_abs_imbalance_tagged")),
                ...
                "priority_score": _priority(
                    ...
                    _safe_float(card.get("avg_abs_imbalance_tagged")),
                ),
            }
        )
    rows.sort(key=lambda row: (float(row["priority_score"]), float(row["avg_abs_imbalance_tagged"])), reverse=True)
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build a multi-symbol book proxy pressure watchlist.")
    ...
    p.add_argument("--out-json", default="reports/BOOK_PROXY_PRESSURE_WATCHLIST.json")
    p.add_argument("--out-md", default="reports/BOOK_PROXY_PRESSURE_WATCHLIST.md")
