...
class OrderRouterIdempotencyTests(unittest.TestCase):
    ...
    def test_create_order_blocks_duplicate_submit_when_submitted_stage_exists(self):
        sym_raw = "BTC/USDT:USDT"
        sym = "BTC/USDT"
        market = {"contract": True}
        ex = DummyEx(markets={sym_raw: market}, market_meta=market, dual_side=True)
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            corr = "ENTRY-BTC-SUBMITTED"
            coid = "ENTRY_BTC_SUBMITTED_1"
            from eclipse_scalper.execution import intent_ledger  # noqa: E402
            ...
            info = dict((res or {}).get("info") or {})
            self.assertTrue(bool(info.get("intent_pending_unknown", False)))
            self.assertEqual(str(info.get("stage") or ""), "SUBMITTED")
    ...
    def test_restart_recovers_submitted_from_journal_and_blocks_duplicate(self):
        sym_raw = "BTC/USDT:USDT"
        sym = "BTC/USDT"
        market = {"contract": True}
        ex = DummyEx(markets={sym_raw: market}, market_meta=market, dual_side=True)
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            corr = "ENTRY-BTC-RESTART-SUB"
            coid = "ENTRY_BTC_RESTART_SUB_1"
            from eclipse_scalper.execution import intent_ledger  # noqa: E402
