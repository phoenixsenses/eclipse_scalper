...
from tools.volatility_burst_alerts import build_payload as build_alert_payload
from tools.volatility_burst_state import build_state_payload
...
def _priority(state_level: str, freshness_status: str, recent_alert_count: int, avg_abs_ret_1_tagged: float) -> float:
    base = {"quiet": 0.0, "elevated": 100.0, "severe": 200.0}.get(str(state_level), 0.0)
    if str(freshness_status) != "fresh":
        base -= 75.0
    return base + min(50.0, float(recent_alert_count)) + min(50.0, float(avg_abs_ret_1_tagged) * 10000.0)
...
def _build_banner(summary: Dict[str, Any], top_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "headline": f"Volatility burst watchlist top={symbol or 'none'} level={level} freshness={freshness} action={action}",
    ...
    *,
    ...
    for raw_symbol in symbols:
        ...
        alert_payload = build_alert_payload(
            ...
            out_json=f"reports/{symbol}_VOLATILITY_BURST_ALERTS.json",
            out_md=f"reports/{symbol}_VOLATILITY_BURST_ALERTS.md",
        )
        state_payload = build_state_payload(
            alert_payload=alert_payload,
            source_json=f"reports/{symbol}_VOLATILITY_BURST_ALERTS.json",
            out_json=f"reports/{symbol}_VOLATILITY_BURST_STATE.json",
            out_md=f"reports/{symbol}_VOLATILITY_BURST_STATE.md",
        )
        ...
        rows.append(
            {
                ...
                "dominant_direction": str(state.get("dominant_direction") or "FLAT"),
                ...
                "avg_abs_ret_1_tagged": _safe_float(card.get("avg_abs_ret_1_tagged")),
                ...
                "priority_score": _priority(
                    ...
                    _safe_float(card.get("avg_abs_ret_1_tagged")),
                ),
            }
        )
    rows.sort(key=lambda row: (float(row["priority_score"]), float(row["avg_abs_ret_1_tagged"])), reverse=True)
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build a multi-symbol volatility burst watchlist.")
    ...
    p.add_argument("--out-json", default="reports/VOLATILITY_BURST_WATCHLIST.json")
    p.add_argument("--out-md", default="reports/VOLATILITY_BURST_WATCHLIST.md")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    lines = [
        ...
        "",
        "| symbol | state_level | freshness_status | recommended_action | dominant_direction | recent_alert_count | high_count | medium_count | avg_abs_ret_1_tagged | priority_score |",
        "|---|---|---|---|---|---:|---:|---:|---:|---:|",
    ]
    for row in payload["rows"]:
        lines.append(
            f"| {row['symbol']} | {row['state_level']} | {row['freshness_status']} | {row['recommended_action']} | "
            f"{row['dominant_direction']} | {int(row['recent_alert_count'])} | {int(row['high_count'])} | "
            f"{int(row['medium_count'])} | {float(row['avg_abs_ret_1_tagged']):.6f} | {float(row['priority_score']):.2f} |"
        )
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
