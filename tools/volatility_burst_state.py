...
def _compute_state(summary: Dict[str, Any]) -> Tuple[str, List[str]]:
    ...
    reasons: List[str] = []
    if high_count >= 2:
        reasons.append("multiple_high_volatility_bursts")
    if recent_alert_count >= 8:
        reasons.append("dense_volatility_burst_cluster")
    if avg_abs_ret >= 0.0015:
        reasons.append("extreme_avg_abs_return")
    if reasons:
        return ("severe", reasons)
    if recent_alert_count >= 3:
        reasons.append("recent_volatility_burst_cluster")
    if tagged_rate >= 0.03:
        reasons.append("elevated_volatility_burst_rate")
...
def build_state_payload(
    *,
    ...
    payload = {
        ...
        "dashboard_summary": (
            f"{str(alert_payload.get('symbol') or '').upper()} {state} volatility burst, "
            f"{_safe_int(summary.get('recent_alert_count'))} recent alerts, freshness {str(freshness['status'])}."
        ),
        "notification_text": (
            f"[volatility-burst] symbol={str(alert_payload.get('symbol') or '').upper()} level={state} "
            f"freshness={str(freshness['status'])} direction={dominant_direction} recent_alerts={_safe_int(summary.get('recent_alert_count'))} "
            f"avg_abs_ret={_safe_float(summary.get('avg_abs_ret_1_tagged')):.6f} action={recommended_action}"
        ),
        "recommended_action": recommended_action,
        "card": {
            "headline": f"{str(alert_payload.get('symbol') or '').upper()} volatility burst {state}",
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compress volatility burst alerts into a runtime-friendly state payload.")
    p.add_argument("--alerts-json", default="reports/VOLATILITY_BURST_ALERTS.json")
    p.add_argument("--out-json", default="reports/VOLATILITY_BURST_STATE.json")
    p.add_argument("--out-md", default="reports/VOLATILITY_BURST_STATE.md")
