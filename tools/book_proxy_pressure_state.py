...
def _compute_state(summary: Dict[str, Any]) -> Tuple[str, List[str]]:
    ...
    avg_abs_imbalance = _safe_float(summary.get("avg_abs_imbalance_tagged"))
    reasons: List[str] = []
    if high_count >= 2:
        reasons.append("multiple_high_proxy_pressure")
    if recent_alert_count >= 8:
        reasons.append("dense_proxy_pressure_cluster")
    if avg_abs_imbalance >= 0.80:
        reasons.append("extreme_proxy_imbalance")
    if reasons:
        return ("severe", reasons)
    if recent_alert_count >= 3:
        reasons.append("recent_proxy_pressure_cluster")
    if tagged_rate >= 0.03:
        reasons.append("elevated_proxy_pressure_rate")
    if avg_abs_imbalance >= 0.65:
        reasons.append("high_proxy_imbalance")
    if reasons:
...
def _compute_freshness(*, latest_alert_ts_ms: int, now_ts_ms: int, bucket_sec: int) -> Dict[str, Any]:
    ...
    return {"status": "fresh" if age_sec <= float(stale_after_sec) else "stale", "age_sec": age_sec, "stale_after_sec": int(stale_after_sec)}
...
def build_state_payload(
    *,
    ...
    latest_alert_ts_ms = _safe_int((alerts[0] if alerts else {}).get("ts_ms"))
    effective_now_ts_ms = _safe_int(now_ts_ms, int(time.time() * 1000.0))
    freshness = _compute_freshness(latest_alert_ts_ms=latest_alert_ts_ms, now_ts_ms=effective_now_ts_ms, bucket_sec=_safe_int(alert_payload.get("bucket_sec"), 5))
    recommended_action = _recommended_action(state=state, freshness_status=str(freshness["status"]))
    side_bias_counts = dict(summary.get("side_bias_counts") or {})
    primary_side_bias = max(side_bias_counts, key=side_bias_counts.get) if side_bias_counts else "NEUTRAL"
    payload = {
        ...
        "state": {
            "level": state,
            "reasons": reasons,
            "primary_side_bias": str(primary_side_bias),
            "freshness": dict(freshness),
        },
        "dashboard_summary": (
            f"{str(alert_payload.get('symbol') or '').upper()} {state} book proxy pressure, "
            f"{_safe_int(summary.get('recent_alert_count'))} recent alerts, freshness {str(freshness['status'])}."
        ),
        "notification_text": (
            f"[book-proxy-pressure] symbol={str(alert_payload.get('symbol') or '').upper()} level={state} "
            f"freshness={str(freshness['status'])} side_bias={primary_side_bias} recent_alerts={_safe_int(summary.get('recent_alert_count'))} "
            f"avg_abs_imbalance={_safe_float(summary.get('avg_abs_imbalance_tagged')):.4f} action={recommended_action}"
        ),
        "recommended_action": recommended_action,
        "card": {
            "headline": f"{str(alert_payload.get('symbol') or '').upper()} book proxy pressure {state}",
            ...
            "avg_abs_imbalance_tagged": _safe_float(summary.get("avg_abs_imbalance_tagged")),
        ...
        "summary_snapshot": {
            ...
            "avg_trade_intensity_tagged": _safe_float(summary.get("avg_trade_intensity_tagged")),
            "avg_spread_tagged": _safe_float(summary.get("avg_spread_tagged")),
            "side_bias_counts": side_bias_counts,
        },
    }
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Compress book proxy pressure alerts into a runtime-friendly state payload.")
    p.add_argument("--alerts-json", default="reports/BOOK_PROXY_PRESSURE_ALERTS.json")
    p.add_argument("--out-json", default="reports/BOOK_PROXY_PRESSURE_STATE.json")
    p.add_argument("--out-md", default="reports/BOOK_PROXY_PRESSURE_STATE.md")
