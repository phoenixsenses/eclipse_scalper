...
import type { VolumeVacuumState } from "../api/types";
...
export default function VolumeVacuumCard({ data }: { data: VolumeVacuumState | null }) {
  if (!data || !data.available) {
    return (
      <div style={{ padding: "12px 16px", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, opacity: 0.6 }}>
        <strong>Volume Vacuum</strong>
  ...
  return (
    <div style={{ padding: "12px 16px", background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 8 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <strong style={{ color: colors.text }}>Volume Vacuum</strong>
        <span style={{
      ...
      <div style={{ marginTop: 8, fontSize: 12 }}>
        <div style={{ color: "#9ca3af" }}>Avg Intensity (thin market)</div>
        <div style={{ fontWeight: 600 }}>{fmtNum(card.avg_trade_intensity_tagged, 1)}</div>
      </div>
