...
import type { VolatilityBurstState } from "../api/types";
...
export default function VolatilityBurstCard({ data }: { data: VolatilityBurstState | null }) {
  if (!data || !data.available) {
    return (
      <div style={{ padding: "12px 16px", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, opacity: 0.6 }}>
        <strong>Volatility Burst</strong>
  ...
  return (
    <div style={{ padding: "12px 16px", background: colors.bg, border: `1px solid ${colors.border}`, borderRadius: 8 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <strong style={{ color: colors.text }}>Volatility Burst</strong>
        <span style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12, marginTop: 8 }}>
        <div>
          <div style={{ color: "#9ca3af" }}>Avg Vol (1s)</div>
