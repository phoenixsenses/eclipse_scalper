"""ASOF Batch 4 consumer migration parity #1 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V7).

`tools/research_s34_wave_absorption.py`'s `book_features_at` (an
`ORDER BY ts_ms DESC LIMIT 1` as-of point lookup returning a 9-column
derived feature dict) is migrated to `book_features_at_v2` (reader-backed).
`book_features_at` is kept unchanged as the parity oracle.

Call-site audit: `symbol` is a genuine function parameter, but this file's
sole call site (in `build_rows`) always passes the hardcoded module
constant `SYMBOL="ETHUSDT"` -- confirmed by direct source inspection, never
overridden. book_ticker's only archived partition is SOLUSDT/2026-04 --
ETHUSDT has NO archive coverage, so real production use of this file
resolves SQLITE_ONLY only. Archive-only and hybrid coverage are therefore
proven with synthetic fixtures, NOT real production smoke.
"""
...
import tools.research_s34_wave_absorption as mod
from tools.research_s34_wave_absorption import book_features_at, book_features_at_v2
...
def _old(ts, max_staleness_sec=3600):
    conn, log = SRC.open_read_only(REAL_SOURCE_DB)
    try:
        return book_features_at(conn, "ETHUSDT", ts, max_staleness_sec)
    finally:
...
# --- real production smoke: hardcoded-call-site ETHUSDT resolves SQLITE_ONLY ---
...
def test_v2_parity_ethusdt_sqlite_only_exact_hit():
    ...
    old = _old(max_ts)
    new = book_features_at_v2(_root(), "ETHUSDT", max_ts, 3600, source_db_path=REAL_SOURCE_DB)
    assert old is not None and new is not None
    assert new == old  # full derived-dict parity (bid/ask/mid/spread/imbalance/depth)
...
def test_v2_parity_ethusdt_between_timestamps():
    ...
    old = _old(q)
    new = book_features_at_v2(_root(), "ETHUSDT", q, 3600, source_db_path=REAL_SOURCE_DB)
    assert old is not None and new is not None
    assert new == old
    assert int(new["book_ts_ms"]) <= q


def test_v2_staleness_reject_parity():
    _needs_real_db()
    q = int(_eth_max_ts()) - 1234
    old = _old(q, max_staleness_sec=0)  # 0-sec budget: any positive age rejects
    new = book_features_at_v2(_root(), "ETHUSDT", q, 0, source_db_path=REAL_SOURCE_DB)
    assert old == new  # both None (or both same non-None if exact hit with 0 age)
...
def test_v2_repeated_run_determinism():
    ...
    max_ts = _eth_max_ts()
    root = _root()
    vals = [book_features_at_v2(root, "ETHUSDT", max_ts, 3600, source_db_path=REAL_SOURCE_DB) for _ in range(3)]
...
def test_source_mutation_zero():
    ...
    book_features_at_v2(root, "ETHUSDT", _eth_max_ts(), 3600, source_db_path=REAL_SOURCE_DB)
...
def test_only_call_site_uses_hardcoded_symbol_constant():
    import inspect
    src = inspect.getsource(mod.build_rows)
    assert "book_features_at_v2(root, SYMBOL," in src


# ---------------------------------------------------------------------------
# Synthetic fixtures: since this file's ETHUSDT call site never touches a
# real archive, archive-only / hybrid / trust coverage is proven here. The
# migrated helper itself is symbol-generic; only THIS file's call site fixes
# ETHUSDT.
# ---------------------------------------------------------------------------
...
def _rows(start_id, start_ms, n, symbol="ETHUSDT", step_ms=1000, bid0=3000.0, depth=1000.0):
    return [(start_id + i, start_ms + i * step_ms, symbol, bid0 + i, 1.0, bid0 + i + 1.0, 1.0,
             bid0 + i + 0.5, 0.005, 0.1, depth) for i in range(n)]
...
def test_archive_only_lookup_synthetic(tmp_path):
    ...
    _catalog(root, [entry])
    val = book_features_at_v2(root, "ETHUSDT", APR + 5000, 3600)
    assert val is not None and val["book_ts_ms"] == APR + 5000
    assert val["bid"] == 3005.0
    assert val["total_top_depth_usd"] == val["bid_depth_usd"] + val["ask_depth_usd"]
...
def test_hybrid_fallback_synthetic(tmp_path):
    ...
    val = book_features_at_v2(root, "ETHUSDT", q, 3600, source_db_path=db)
    assert val is not None and val["book_ts_ms"] == MAY - 1000
...
def test_exact_hit_and_between_synthetic(tmp_path):
    ...
    exact = book_features_at_v2(root, "ETHUSDT", MAY + 2000, 3600, source_db_path=db)
    assert exact["book_ts_ms"] == MAY + 2000
    between = book_features_at_v2(root, "ETHUSDT", MAY + 2000 + 300, 3600, source_db_path=db)
    assert between["book_ts_ms"] == MAY + 2000
...
def test_empty_no_prior_row_synthetic(tmp_path):
    ...
    assert book_features_at_v2(root, "ETHUSDT", MAY - 3600_000, 3600, source_db_path=db) is None
...
def test_tie_break_parity_synthetic(tmp_path):
    ...
    c.close()
    result = book_features_at_v2(root, "ETHUSDT", tied, 3600, source_db_path=db)
    assert oracle_id == 3
...
def test_symbol_mismatch_safety_synthetic(tmp_path):
    ...
    eth = book_features_at_v2(root, "ETHUSDT", MAY, 3600, source_db_path=db)
...
def test_nullable_bid_depth_usd_fallback_parity_synthetic(tmp_path):
    # When bid_depth_usd is NULL, both oracle and v2 fall back to bid*bid_qty.
    root = str(tmp_path / "r")
    os.makedirs(root)
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
    _sqlite(db, [(1, MAY, "ETHUSDT", 3000.0, 2.0, 3001.0, 1.0, 3000.5, 0.005, 0.1, None)])
    val = book_features_at_v2(root, "ETHUSDT", MAY, 3600, source_db_path=db)
    assert val["bid_depth_usd"] == 3000.0 * 2.0
...
def test_trust_failure_fails_closed_synthetic(tmp_path):
    ...
    with pytest.raises(RR.ArchiveTrustError):
        book_features_at_v2(root, "ETHUSDT", APR + 5000, 3600)
