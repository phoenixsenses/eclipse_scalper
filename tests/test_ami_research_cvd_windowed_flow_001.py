"""G2-CVD-PRIMARY-LONG-GOVERNED-EXECUTION-V1 -- pre-execution validation.

Three groups:
1. Pure statistics (synthetic data, no DB) -- OLS/cluster-robust SE/VIF/
   collinearity policy/verdict rule/listwise deletion.
2. Real-data (mode=ro) population/split/identity reproduction -- proves the
   module's TEST-outcome-blind functions reproduce the frozen
   preregistration exactly, without ever reading endpoint_return_bps/mfe_bps.
...
Run: pytest tests/test_ami_research_cvd_windowed_flow_001.py --basetemp <scratchpad> -p no:cacheprovider
"""
...
import ami.research.cvd_windowed_flow_001 as m
...
def test_ols_exact_recovery_no_noise():
    ...
    X[:, 6] = rng.normal(0, 1, n)
    true_beta = np.array([1.5, 3.0, -2.0, 0.5, -0.5, 0.25, 0.75])
...
def test_ols_cluster_grouping_reduces_effective_g():
    ...
    # 10 clusters of 4 rows each -- G must be 10, not 40
...
def test_vif_high_for_collinear_columns_low_for_independent():
    ...
    n = 100
    x1 = rng.normal(0, 1, n)
    x_collinear = x1 * 2.0 + rng.normal(0, 0.001, n)  # near-perfect collinearity with x1
    x_independent = rng.normal(0, 1, n)
    X = np.column_stack([np.ones(n), x1, x_collinear, x_independent,
                          rng.integers(0, 2, n), rng.integers(0, 2, n)])
    vifs_raw = {}
    n_, k_ = X.shape
    for j in range(1, k_):
        y_j = X[:, j]
        other = [c for c in range(k_) if c != j]
        Xo = X[:, other]
        ...
        vifs_raw[j] = 1.0 / (1.0 - r2) if r2 < 1.0 else float("inf")
    # x1 (col 1) and x_collinear (col 2) should both show high VIF; x_independent (col 3) low
    assert vifs_raw[1] > 10
    assert vifs_raw[2] > 10
    assert vifs_raw[3] < 5
...
def test_collinearity_policy_drop_order_and_session_never_dropped():
    vifs_both_high = {"day_trend_bps": 15.0, "event_notional_per_100k": 12.0,
                       "session_EUROPE": 20.0, "session_US": 20.0, "session_OFF": 20.0}
    ...
    vifs_none_high = {"day_trend_bps": 2.0, "event_notional_per_100k": 3.0, "session_EUROPE": 50.0}
    assert m.apply_collinearity_policy(vifs_none_high) == []  # session never in the drop set


def test_verdict_rule_insufficient_sample():
    verdict, _ = m.apply_verdict_rule(n_test=10, coef=1.0, se=0.1, ci_lo=0.8, ci_hi=1.2, p_value=0.001)
    assert verdict == "INSUFFICIENT_SAMPLE_OR_INCONCLUSIVE"


def test_verdict_rule_ci_too_wide_is_inconclusive():
...
def test_verdict_rule_supports_incremental_association():
    # coef=1.0 per $1M -> per $10M = 10 >= floor(5); CI excludes 0; p<0.05
    verdict, reason = m.apply_verdict_rule(n_test=40, coef=1.0, se=0.3, ci_lo=0.4, ci_hi=1.6, p_value=0.001)
    assert verdict == "EVIDENCE_SUPPORTS_INCREMENTAL_ASSOCIATION"
    assert "CI excludes 0" in reason


def test_verdict_rule_significant_but_below_relevance_floor_is_no_reliable_association():
    # tiny coefficient: per $10M = 0.5 < floor(5), even though CI excludes 0 and p<0.05
    verdict, _ = m.apply_verdict_rule(n_test=40, coef=0.05, se=0.005, ci_lo=0.04, ci_hi=0.06, p_value=0.0001)
    assert verdict == "NO_RELIABLE_ASSOCIATION"
...
def test_build_design_applies_listwise_deletion():
    rows = [
        {"cycle_key": "a", "cvd_notional": 1_000_000.0, "event_notional": 100_000.0,
         "session": "ASIA", "day_trend_bps": 1.0, "endpoint_return_bps": 5.0, "missing_predictor": False},
        {"cycle_key": "b", "cvd_notional": None, "event_notional": 100_000.0,
         "session": "EUROPE", "day_trend_bps": 1.0, "endpoint_return_bps": 5.0, "missing_predictor": True},
        {"cycle_key": "c", "cvd_notional": 2_000_000.0, "event_notional": 100_000.0,
         "session": "US", "day_trend_bps": 1.0, "endpoint_return_bps": None, "missing_predictor": False},
    ]
...
def test_execute_governed_run_blocks_on_identity_mismatch(monkeypatch):
    ...
    try:
        ...
        # nothing consumed/written on a blocked attempt
...
def test_governed_execution_dress_rehearsal_on_disposable_copies():
    ...
    try:
        ...
        assert r1["verdict"] in (
            "EVIDENCE_SUPPORTS_INCREMENTAL_ASSOCIATION", "NO_RELIABLE_ASSOCIATION",
            "INSUFFICIENT_SAMPLE_OR_INCONCLUSIVE", "PROTOCOL_OR_DATA_QUALITY_INVALIDATION",
        )

        # nullifier now consumed exactly once, bound to this experiment_id
        n_rows = knowledge_conn.execute(
        ...
        # exactly one new experiment_registry/experiment_results set; canonical
        # tables unrelated to this experiment are untouched
        ...
        n_rows_again = knowledge_conn.execute(
            "SELECT COUNT(*) FROM epistemic_test_nullifiers WHERE nullifier=?",
            (m.EXPECTED_NULLIFIER,)).fetchone()[0]
        assert n_rows_again == 1  # still exactly one consumption row, not two
    finally:
        canonical_conn.close()
        knowledge_conn.close()
