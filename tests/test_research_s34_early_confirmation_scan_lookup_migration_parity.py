"""ASOF Batch 6 (final) consumer migration parity #2 (BATCH-STORAGE-
ROTATION-RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V9).

`tools/research_s34_early_confirmation_scan.py`'s `book_ticker_at` (an
...
scope for this gate:
  - `mark_at`: dual-direction (before=True is as-of DESC LIMIT 1, before=
    ...
    call sites (in early_window) always pass before=False -- the as-of
    branch is never exercised, and mark_prices is out of scope regardless
...
from tools.research_s34_early_confirmation_scan import book_ticker_at, book_ticker_at_v2
...
def test_mark_at_before_false_only_call_sites():
    import inspect
    src = inspect.getsource(mod.early_window)
    assert src.count("mark_at(source_con,") == 2
    assert "before=False" in src
...
def test_real_fill_cost_uses_v2():
    import inspect
    src = inspect.getsource(mod.real_fill_cost)
    assert "book_ticker_at_v2(root," in src
