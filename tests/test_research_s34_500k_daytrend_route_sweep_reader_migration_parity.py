...
RANGE-READ-CONSUMER-MIGRATION-V9), target 1 of 3.

`tools/research_s34_500k_daytrend_route_sweep.py`'s bounded range read
inside `path_marks()` (`mark_prices`, `ts_ms>=? AND ts_ms<=? ORDER BY
ts_ms ASC` -- INCLUSIVE lower / INCLUSIVE upper) was preserved unchanged
as the parity oracle (`path_marks`) and migrated to `path_marks_v2()`
(reader-backed via `plan_read`/`execute_read`). `path_marks()` is no
longer called from `main`'s live path.

Call-site audit: this file hardcodes `SYMBOL="ETHUSDT"` in the oracle SQL
and never varies it; `path_marks_v2` uses the same module constant. The
one live call site (in `main`) was rerouted to `path_marks_v2(root, ...,
...
Out-of-scope, untouched (this gate is range-read only):
  * `mark_at()` -- forward ASOF (`ORDER BY ts_ms ASC LIMIT 1`), the
    direction `lookup_latest_at_or_before` does NOT support; left on
    direct SQL.
  * `book_ticker_at_v2()` -- already migrated in a prior ASOF gate.
  * `liq_event_features` queries (feature factory DB) -- out-of-allowlist.
"""
...
import tools.research_s34_500k_daytrend_route_sweep as mod
from tools.research_s34_500k_daytrend_route_sweep import path_marks, path_marks_v2
...
def _old(entry_ms):
    """Direct-SQL oracle over the REAL db, with the module's fixed horizon."""
    conn, log = SRC.open_read_only(REAL_SOURCE_DB)
    try:
        return path_marks(conn, entry_ms)
    finally:
...
def test_v2_parity_archive_only_real_window_and_plan_mode():
    ...
    old_rows = _old(entry)
    new_rows = path_marks_v2(_root(), entry, source_db_path=REAL_SOURCE_DB)
...
def test_source_mutation_zero_real():
    ...
    path_marks_v2(root, MAY_ARCHIVE_START_MS, source_db_path=REAL_SOURCE_DB)
...
def test_full_window_parity_synthetic(tmp_path):
    ...
    _sqlite(db, [
        (1, MAY - 1, "ETHUSDT", 2999.0, 0.0, None),          # below lo -- excluded
        (2, MAY, "ETHUSDT", 3000.0, 0.0, None),              # == lo -- INCLUDED
        (3, MAY + 1000, "ETHUSDT", 3001.0, 0.0, None),
        (4, MAY + HORIZON_MS, "ETHUSDT", 3050.0, 0.0, None),  # == hi -- INCLUDED
        (5, MAY + HORIZON_MS + 1, "ETHUSDT", 3060.0, 0.0, None),  # above hi -- excluded
        (6, MAY + 500, "SOLUSDT", 100.0, 0.0, None),          # wrong symbol -- excluded
    ])
    old_rows = _old_synth(db, MAY)
    new_rows = path_marks_v2(root, MAY, source_db_path=db)
    assert new_rows == old_rows
    assert new_rows == [(MAY, 3000.0), (MAY + 1000, 3001.0), (MAY + HORIZON_MS, 3050.0)]


def test_inclusive_lower_boundary_synthetic(tmp_path):
    root = str(tmp_path / "r")
    os.makedirs(root)
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
    ...
    rows = path_marks_v2(root, MAY, source_db_path=db)
    assert rows[0] == (MAY, 3000.0)  # row exactly at lo=MAY is INCLUDED
    assert rows == _old_synth(db, MAY)


def test_inclusive_upper_boundary_synthetic(tmp_path):
    root = str(tmp_path / "r")
    os.makedirs(root)
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
    _sqlite(db, [
        (1, MAY, "ETHUSDT", 3000.0, 0.0, None),
        (2, MAY + HORIZON_MS, "ETHUSDT", 3050.0, 0.0, None),  # == hi
    ])
    rows = path_marks_v2(root, MAY, source_db_path=db)
    assert rows[-1] == (MAY + HORIZON_MS, 3050.0)  # row exactly at hi is INCLUDED
    assert rows == _old_synth(db, MAY)
...
def test_empty_range_synthetic(tmp_path):
    ...
    assert path_marks_v2(root, MAY + HORIZON_MS + 10_000, source_db_path=db) == []
...
def test_determinism_and_batch_size_invariance_synthetic(tmp_path):
    ...
    _sqlite(db, rows)
    d1 = _digest(path_marks_v2(root, MAY, source_db_path=db))
    d2 = _digest(path_marks_v2(root, MAY, source_db_path=db))
    assert d1 == d2

    def _rows(batch_size):
        lo = MAY
        hi = MAY + HORIZON_MS
        plan = RR.plan_read(root, table="mark_prices", symbol="ETHUSDT", start_ms=lo, end_ms=hi + 1)
    ...
    assert _rows(1_000_000) == _rows(7) == _old_synth(db, MAY)
...
def test_oracle_no_longer_called_from_live_path():
    assert "path_marks(" not in inspect.getsource(mod.main)
    assert "path_marks_v2(" in inspect.getsource(mod.main)


def test_mark_at_forward_asof_left_untouched():
    src = inspect.getsource(mod)
    assert "order by ts_ms asc\n        limit 1" in src.lower() or "order by ts_ms asc" in src.lower()
    assert "lookup_latest_at_or_before" in src  # book_ticker ASOF already migrated, unchanged
    # mark_at forward ASOF must remain on direct SQL (unsupported direction)
