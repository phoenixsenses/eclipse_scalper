...
RANGE-READ-CONSUMER-MIGRATION-V9), target 3 of 3.

`tools/research_s34_early_confirmation_scan.py`'s bounded range read inside
`path_marks()` (`mark_prices`, `ts_ms>=? AND ts_ms<=? ORDER BY ts_ms` --
INCLUSIVE lower / INCLUSIVE upper, with a caller-supplied `horizon_sec`)
was preserved unchanged as the parity oracle (`path_marks`) and migrated
to `path_marks_v2()` (reader-backed via `plan_read`/`execute_read`).
`path_marks()` is no longer called from `early_window`/
`simulate_after_wait` (the live path).

Call-site audit: this file hardcodes `SYMBOL="ETHUSDT"` in the oracle SQL
and never varies it; `path_marks_v2` uses the same module constant.
`root`/`source_db_path` were threaded from `main` down through
`scan_scope` -> `build_rows` -> `early_window`/`simulate_after_wait`.
...
`path_marks_v2` passes `start_ms=lo, end_ms=hi+1` (with
`hi=start_ts_ms+horizon_sec*1000`), proven bit-for-bit below.
...
import tools.research_s34_early_confirmation_scan as mod
from tools.research_s34_early_confirmation_scan import path_marks, path_marks_v2
...
def _old(start_ms, horizon_sec):
    conn, log = SRC.open_read_only(REAL_SOURCE_DB)
    try:
        return path_marks(conn, start_ms, horizon_sec)
    finally:
...
def _old_synth(db, start_ms, horizon_sec):
...
def test_v2_parity_archive_only_real_window_and_plan_mode():
    _needs_real_db()
    start = MAY_ARCHIVE_START_MS
    plan = RR.plan_read(_root(), table="mark_prices", symbol=SYMBOL, start_ms=start, end_ms=start + HORIZON_MS + 1)
    assert plan.mode == "ARCHIVE_ONLY"
    old_rows = _old(start, HORIZON_SEC)
    new_rows = path_marks_v2(_root(), start, HORIZON_SEC, source_db_path=REAL_SOURCE_DB)
...
def test_v2_parity_hybrid_real_boundary_and_plan_mode():
    _needs_real_db()
    start = MAY_ARCHIVE_END_MS - HORIZON_MS // 2
    plan = RR.plan_read(_root(), table="mark_prices", symbol=SYMBOL, start_ms=start, end_ms=start + HORIZON_MS + 1)
...
def test_source_mutation_zero_real():
    ...
    path_marks_v2(root, MAY_ARCHIVE_START_MS, HORIZON_SEC, source_db_path=REAL_SOURCE_DB)
    assert os.path.getmtime(idx) == before


# ---------------------------------------------------------------------------
# Synthetic SQLITE_ONLY parity, boundaries, symbol filter, empty, horizon
# ---------------------------------------------------------------------------
...
def test_full_window_parity_synthetic(tmp_path):
    ...
    old_rows = _old_synth(db, MAY, HORIZON_SEC)
    new_rows = path_marks_v2(root, MAY, HORIZON_SEC, source_db_path=db)
    assert new_rows == old_rows
    assert new_rows == [(MAY, 3000.0), (MAY + 1000, 3001.0), (MAY + HORIZON_MS, 3050.0)]


def test_horizon_parameter_parity_synthetic(tmp_path):
    """The window's upper bound tracks the caller's `horizon_sec` exactly."""
    root = str(tmp_path / "r")
    os.makedirs(root)
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
    _sqlite(db, [
        ...
        (2, MAY + 60_000, "ETHUSDT", 3010.0, 0.0, None),
        (3, MAY + 120_000, "ETHUSDT", 3020.0, 0.0, None),
    ])
    for h in (30, 60, 90, 120):
        old_rows = _old_synth(db, MAY, h)
        new_rows = path_marks_v2(root, MAY, h, source_db_path=db)
        assert new_rows == old_rows
...
def test_boundaries_inclusive_synthetic(tmp_path):
    ...
    rows = path_marks_v2(root, MAY, HORIZON_SEC, source_db_path=db)
    assert rows[0] == (MAY, 3000.0)
    assert rows[-1] == (MAY + HORIZON_MS, 3050.0)
    assert rows == _old_synth(db, MAY, HORIZON_SEC)


def test_symbol_filter_synthetic(tmp_path):
    root = str(tmp_path / "r")
    os.makedirs(root)
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
    ...
    rows = path_marks_v2(root, MAY, HORIZON_SEC, source_db_path=db)
    assert rows == _old_synth(db, MAY, HORIZON_SEC) == [(MAY, 3000.0), (MAY + 500, 3001.0)]
...
def test_empty_range_synthetic(tmp_path):
    ...
    assert path_marks_v2(root, MAY + HORIZON_MS + 10_000, HORIZON_SEC, source_db_path=db) == []
...
def test_determinism_and_batch_size_invariance_synthetic(tmp_path):
    ...
    _sqlite(db, rows)
    d1 = _digest(path_marks_v2(root, MAY, HORIZON_SEC, source_db_path=db))
    d2 = _digest(path_marks_v2(root, MAY, HORIZON_SEC, source_db_path=db))
    assert d1 == d2
    ...
    assert _rows(1_000_000) == _rows(7) == _old_synth(db, MAY, HORIZON_SEC)
...
def test_trust_failure_fails_closed_synthetic(tmp_path):
    ...
    with pytest.raises(RR.ArchiveTrustError):
        path_marks_v2(root, MAY, HORIZON_SEC)
...
def test_oracle_no_longer_called_from_live_path():
    for fn in (mod.early_window, mod.simulate_after_wait):
