"""ASOF Batch 5 consumer migration parity #3 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V8).

`tools/research_s34_cluster_geometry_features.py`'s `book_ticker_at` (an
...
Call-site audit: symbol is HARDCODED to the module constant
`SYMBOL="ETHUSDT"` (this file never varies it -- confirmed by direct
source inspection). book_ticker's only archived partition is
SOLUSDT/2026-04 -- ETHUSDT has NO archive coverage, so real production use
resolves SQLITE_ONLY only. Archive-only and hybrid coverage are proven
with synthetic fixtures.

This file's `real_fill_net` was the ONLY place `source_con` was used for
book_ticker; after migration the direct source connection main() used to
open is fully dead and is no longer opened (mirrors the ASOF Batch 3
preliq_detector dead-connection drop).

Separately, `populate_geometry()` writes derived columns to
`liq_event_features` in `data/s34_feature_factory.db` (NOT the protected
source `microstructure.db`, and not the archive/catalog) -- this is
pre-existing behavior in `main()`, which this gate never runs (per the
"no main() is ever run" discipline shared by every migration gate to
date); it is untouched and out of scope for this ASOF gate.
"""
...
import tools.research_s34_cluster_geometry_features as mod
from tools.research_s34_cluster_geometry_features import book_ticker_at, book_ticker_at_v2
...
def test_v2_parity_ethusdt_between_timestamps():
    ...
    old = _old(q)
    new = book_ticker_at_v2(_root(), q, source_db_path=REAL_SOURCE_DB)
    assert old is not None and new is not None
    assert new == old
...
def test_source_mutation_zero():
    ...
    book_ticker_at_v2(root, _eth_max_ts(), source_db_path=REAL_SOURCE_DB)
...
def test_dead_source_con_dropped():
    import inspect
    src = inspect.getsource(mod.main)
    assert "sqlite3.connect(SOURCE_DB" not in src  # dead source connection removed
    assert "resolve_production_root" in src
    assert "FROM book_ticker" not in inspect.getsource(mod.real_fill_net)


def test_populate_geometry_untouched_and_out_of_scope():
    # Confirms this pre-existing feature-factory schema write (unrelated to
    # this ASOF gate) is unchanged and remains gated behind main(), which
    # this test suite never calls.
    import inspect
    src = inspect.getsource(mod.populate_geometry)
    assert "update liq_event_features" in src
    assert "microstructure.db" not in src


def test_real_fill_net_no_longer_needs_connection():
    import inspect
    sig = inspect.signature(mod.real_fill_net)
    assert list(sig.parameters)[0] == "root"
...
def test_symbol_mismatch_safety_synthetic(tmp_path):
    ...
    eth = book_ticker_at_v2(root, MAY, source_db_path=db)
