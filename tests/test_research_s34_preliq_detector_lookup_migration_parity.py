"""ASOF Batch 3 consumer migration parity #3 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V6).

`tools/research_s34_preliq_detector.py`'s `_book_at_or_before` (an
`ORDER BY ts_ms DESC LIMIT 1` as-of point lookup selecting 9 columns,
incl. the nullable `bid_depth_usd`) is migrated to `_book_at_or_before_v2`
(reader-backed). `_book_at_or_before` is kept unchanged as the parity
oracle.
...
coverage, so real production use resolves SQLITE_ONLY only. Archive-only and
hybrid coverage are proven with synthetic fixtures.

This file had NO other book_ticker query, so migrating `_book_at_or_before`
made the direct `micro` source connection fully dead; `main()` no longer
opens it (mirrors the ASOF Batch 2 counter_regime dead-connection drop).
"""
...
import tools.research_s34_preliq_detector as mod
from tools.research_s34_preliq_detector import _book_at_or_before, _book_at_or_before_v2
...
# --- real production smoke: ETHUSDT resolves SQLITE_ONLY; helper == oracle ---
...
def test_v2_parity_ethusdt_sqlite_only_exact_hit():
    ...
    assert dict(old) == new  # 9-column value + selected-column + tie-break parity
...
def test_dead_micro_connection_dropped():
    import inspect
    src = inspect.getsource(mod.main)
    assert "sqlite3.connect(MICRO_DB" not in src  # dead source connection removed
    assert "resolve_production_root" in src
    # no direct book_ticker read remains anywhere in the main path
    assert "FROM book_ticker" not in inspect.getsource(mod._row_features)
...
def test_archive_only_lookup_synthetic(tmp_path):
    ...
    assert val["bid_price"] == 3005.0 and val["bid_depth_usd"] == 1000.0
...
def test_nullable_bid_depth_usd_parity_synthetic(tmp_path):
    # bid_depth_usd is nullable; the dict must carry None through (as the
    # oracle Row would), so downstream `float(now["bid_depth_usd"] or 0.0)`
    # behaves identically.
    root = str(tmp_path / "r")
    os.makedirs(root)
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
    _sqlite(db, [(1, MAY, "ETHUSDT", 3000.0, 1.0, 3001.0, 1.0, 3000.5, 0.005, 0.1, None)])
    val = _book_at_or_before_v2(root, MAY, source_db_path=db)
    assert val["bid_depth_usd"] is None
...
def test_tie_break_parity_synthetic(tmp_path):
    ...
    assert oracle_id == 3
    assert result["bid_price"] == 3020.0
