"""ASOF Batch 2 consumer migration parity #2 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V5).

`tools/research_s34_counter_regime_realfill.py`'s `book_at` (an
`ORDER BY ts_ms DESC LIMIT 1` staleness-checked lookup) is migrated to
`book_at_v2` (reader-backed). `book_at` is kept unchanged as the
parity oracle.

Call-site audit: `book_at`'s `symbol` parameter is always invoked with
`row["symbol"]`, and every row comes from `load_candidate()`, whose SQL
filters `f.symbol=?` bound to the module-level constant
`SYMBOL="ETHUSDT"` (confirmed by direct source inspection -- this
constant is never overridden anywhere in the file). book_ticker's only
archived partition is SOLUSDT/2026-04 -- ETHUSDT has NO archive
coverage at all, so real production use of this file resolves
SQLITE_ONLY only. Archive-only and hybrid coverage for this file are
therefore proven with synthetic fixtures, NOT real production smoke
(same pattern as the btc_microtrend_eth_quality pilot). Skips if the
...
from tools.research_s34_counter_regime_realfill import book_at, book_at_v2
...
def _old(symbol, ts):
    ...
    try:
        return book_at(conn, symbol, ts)
    finally:
        SRC.assert_read_only_session_clean(log)
        conn.close()


# --- real production smoke: confirms this file's real symbol is ETHUSDT-only
# and that resolves SQLITE_ONLY (no book_ticker archive for ETHUSDT) ---

def test_book_at_v2_parity_ethusdt_sqlite_only_confirms_no_archive():
    conn, log = SRC.open_read_only(REAL_SOURCE_DB)
    try:
    ...
    assert plan.mode == "SQLITE_ONLY"  # confirms the real-usage claim: no ETHUSDT archive exists
    old = _old("ETHUSDT", max_ts)
    new = book_at_v2(_root(), "ETHUSDT", max_ts, source_db_path=REAL_SOURCE_DB)
    assert old is not None
    assert new == old


def test_book_at_v2_repeated_run_determinism():
    conn, log = SRC.open_read_only(REAL_SOURCE_DB)
    try:
        max_ts = conn.execute("SELECT MAX(ts_ms) FROM book_ticker WHERE symbol='ETHUSDT'").fetchone()[0]
    finally:
        ...
        conn.close()
    root = _root()
    vals = [book_at_v2(root, "ETHUSDT", max_ts, source_db_path=REAL_SOURCE_DB) for _ in range(3)]
    assert vals[0] == vals[1] == vals[2]


def test_book_at_v2_source_mutation_zero():
    root = _root()
    ...
    try:
        ...
        SRC.assert_read_only_session_clean(log)
        conn.close()
    book_at_v2(root, "ETHUSDT", max_ts, source_db_path=REAL_SOURCE_DB)
    assert os.path.getmtime(idx) == before


def test_no_full_reverify_referenced():
    import inspect
    import tools.research_s34_counter_regime_realfill as mod
...
# archive, archive-only and hybrid coverage is proven here, symbol-generic
# (the migrated helper itself supports any symbol; only THIS file's call
# site happens to always pass ETHUSDT).
# ---------------------------------------------------------------------------
...
def test_archive_only_lookup_synthetic(tmp_path):
    ...
    _catalog(root, [entry])
    val = book_at_v2(root, "ETHUSDT", APR + 5000)
...
def test_hybrid_fallback_synthetic(tmp_path):
    """Archive's last row leads right up to its partition end, and the
    query lands just after the boundary (live-side, but SQLite is
    empty) -- close enough in time that the staleness check still
    accepts the archive-side fallback row."""
    root = str(tmp_path / "r")
    os.makedirs(root)
    entry = _build_archive(root, symbol="ETHUSDT", partition_start_ms=APR, partition_end_ms=MAY,
                            rows=_rows(1, MAY - 3000, 3, step_ms=1000))  # rows at MAY-3000, MAY-2000, MAY-1000
    _catalog(root, [entry])
    db = str(tmp_path / "s.sqlite")
    _sqlite(db, [])  # live empty -> query after archive end falls back to archive
    q = MAY + 500  # 1500ms after the archive's last row -- within the 5s staleness budget
    ...
    val = book_at_v2(root, "ETHUSDT", q, source_db_path=db)
...
def test_exact_hit_and_between_and_staleness_synthetic(tmp_path):
    ...
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
    _sqlite(db, _rows(1, MAY, 3, step_ms=1000))  # rows at MAY, MAY+1000, MAY+2000 -- nothing later
    exact = book_at_v2(root, "ETHUSDT", MAY + 2000, source_db_path=db)
    assert exact["ts_ms"] == MAY + 2000
    between = book_at_v2(root, "ETHUSDT", MAY + 2000 + 300, source_db_path=db)
    assert between["ts_ms"] == MAY + 2000
    stale = book_at_v2(root, "ETHUSDT", MAY + 2000 + 5001, source_db_path=db)
...
def test_empty_no_prior_row_synthetic(tmp_path):
    ...
    assert book_at_v2(root, "ETHUSDT", MAY - 3600_000, source_db_path=db) is None
...
def test_tie_break_parity_synthetic(tmp_path):
    ...
    c.close()
    result = book_at_v2(root, "ETHUSDT", tied, source_db_path=db)
    assert oracle_id == 3
...
def test_symbol_mismatch_safety_synthetic(tmp_path):
    ...
    eth = book_at_v2(root, "ETHUSDT", MAY, source_db_path=db)
    sol = book_at_v2(root, "SOLUSDT", MAY, source_db_path=db)
...
def test_trust_failure_fails_closed_synthetic(tmp_path):
    ...
    with pytest.raises(RR.ArchiveTrustError):
        book_at_v2(root, "ETHUSDT", APR + 5000)
