"""ASOF Batch 5 consumer migration parity #2 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V8).

`tools/research_s34_trailing_oos_realfill.py`'s `book_ticker_at` (an
`ORDER BY ts_ms DESC LIMIT 1` as-of point lookup returning
`{ts_ms,bid,ask,mid}`) is migrated to `book_ticker_at_v2` (reader-backed).
`book_ticker_at` is kept unchanged as the parity oracle.

Call-site audit: `symbol` is a genuine function parameter, but this file's
sole call site (`real_fill_cost`, called twice per event) always passes the
hardcoded module constant `SYMBOL="ETHUSDT"` -- confirmed by direct source
inspection, never overridden. book_ticker's only archived partition is
...
are proven with synthetic fixtures.

This file has two OTHER queries left on direct SQL, deliberately out of
scope for this gate:
  - `mark_at`: dual-direction (before=True is as-of DESC LIMIT 1, before=
    False is forward "at-or-after" ASC LIMIT 1), but this file's only two
    call sites always pass before=False -- the as-of branch is never
    exercised in practice, and mark_prices is out of scope regardless
    (belongs to the range-read track).
  - `path_marks`: bounded range read on mark_prices.
"""
...
from tools.research_s34_trailing_oos_realfill import book_ticker_at, book_ticker_at_v2
...
# --- real production smoke: this file's real symbol is ETHUSDT-only and
# resolves SQLITE_ONLY (no book_ticker archive for ETHUSDT) ---
...
def test_v2_parity_ethusdt_between_timestamps():
    ...
    old = _old("ETHUSDT", q)
    new = book_ticker_at_v2(_root(), "ETHUSDT", q, source_db_path=REAL_SOURCE_DB)
    assert old is not None and new is not None
    assert new == old
...
def test_v2_repeated_run_determinism():
    ...
    max_ts = _eth_max_ts()
    root = _root()
    vals = [book_ticker_at_v2(root, "ETHUSDT", max_ts, source_db_path=REAL_SOURCE_DB) for _ in range(3)]
...
def test_source_mutation_zero():
    ...
    book_ticker_at_v2(root, "ETHUSDT", _eth_max_ts(), source_db_path=REAL_SOURCE_DB)
...
def test_mark_at_before_false_only_call_sites():
    import inspect
    # Both real call sites always pass before=False -- the DESC (as-of)
    # branch of mark_at is never actually exercised in this file.
    src1 = inspect.getsource(mod.simulate_current_be30)
    src2 = inspect.getsource(mod.simulate_trail_half_mfe)
    assert "mark_at(source_con, SYMBOL, entry_target, before=False)" in src1
    assert "mark_at(source_con, SYMBOL, entry_target, before=False)" in src2
    assert "RR.lookup_latest_at_or_before(" not in inspect.getsource(mod.mark_at)


def test_path_marks_left_on_direct_sql_out_of_scope():
    import inspect
    src = inspect.getsource(mod.path_marks)
    assert "mark_prices" in src and "ts_ms>=?" in src and "ts_ms<=?" in src
    assert "RR.lookup_latest_at_or_before(" not in src


def test_real_fill_cost_uses_v2_with_hardcoded_symbol_constant():
    import inspect
    src = inspect.getsource(mod.real_fill_cost)
    assert "book_ticker_at_v2(root, SYMBOL," in src
...
def test_archive_only_lookup_synthetic(tmp_path):
    ...
    _catalog(root, [entry])
    val = book_ticker_at_v2(root, "ETHUSDT", APR + 5000)
...
def test_hybrid_fallback_synthetic(tmp_path):
    ...
    val = book_ticker_at_v2(root, "ETHUSDT", q, source_db_path=db)
...
def test_exact_hit_and_between_synthetic(tmp_path):
    ...
    exact = book_ticker_at_v2(root, "ETHUSDT", MAY + 2000, source_db_path=db)
    assert exact["ts_ms"] == MAY + 2000
    between = book_ticker_at_v2(root, "ETHUSDT", MAY + 2000 + 300, source_db_path=db)
...
def test_empty_no_prior_row_synthetic(tmp_path):
    ...
    assert book_ticker_at_v2(root, "ETHUSDT", MAY - 3600_000, source_db_path=db) is None
...
def test_tie_break_parity_synthetic(tmp_path):
    ...
    c.close()
    result = book_ticker_at_v2(root, "ETHUSDT", tied, source_db_path=db)
    assert oracle_id == 3
...
def test_trust_failure_fails_closed_synthetic(tmp_path):
    ...
    with pytest.raises(RR.ArchiveTrustError):
        book_ticker_at_v2(root, "ETHUSDT", APR + 5000)
