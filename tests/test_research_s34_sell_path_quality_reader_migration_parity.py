"""Range-read consumer migration V5 parity #1 (BATCH-STORAGE-ROTATION-
RETENTION-RANGE-READ-CONSUMER-MIGRATION-V5).

`tools/research_s34_sell_path_quality.py`'s `sim_short` had an inline
...
reference. Structurally identical to V3's sell/buy_reversal files.
...
import tools.research_s34_sell_path_quality as mod
from tools.research_s34_sell_path_quality import _horizon_marks, _horizon_marks_v2
...
def test_all_call_sites_thread_root_and_source_db_path():
    import inspect
    src = inspect.getsource(mod.main)
    assert src.count("sim_short(con,") == 4
    assert src.count("root=root, source_db_path=SOURCE_DB_PATH") == 4
