"""ASOF Batch 5 consumer migration parity #1 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V8).

`tools/research_s34_eth_preliq_executable.py`'s `_book_at_or_before` (an
`ORDER BY ts_ms DESC LIMIT 1` as-of point lookup selecting 8 columns) is
migrated to `_book_at_or_before_v2` (reader-backed). `_book_at_or_before` is
kept unchanged as the parity oracle.

Call-site audit: symbol is HARDCODED directly in the oracle's SQL
(`WHERE symbol='ETHUSDT'`) -- there is no symbol parameter in this function
at all, so it can never vary. book_ticker's only archived partition is
...
`_book_series` (a bounded BETWEEN range read) is left on direct SQL,
deliberately out of scope for this as-of point-lookup gate.
"""
...
import tools.research_s34_eth_preliq_executable as mod
from tools.research_s34_eth_preliq_executable import _book_at_or_before, _book_at_or_before_v2
