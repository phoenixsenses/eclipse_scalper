"""Range-read consumer migration V5 parity #2 (BATCH-STORAGE-ROTATION-
RETENTION-RANGE-READ-CONSUMER-MIGRATION-V5).

`tools/research_s34_sell_regime_analysis.py`'s `build_mark_cache` had an
inline mark_prices range read (looped over multiple symbols); it is
extracted into a named oracle `_mark_prices_range` and migrated to
`_mark_prices_range_v2` (reader-backed via plan_read/execute_read).
`_mark_prices_range` is kept unchanged as the parity reference.

`symbol` is a GENUINE runtime parameter, iterated over the distinct
symbols across ROUTES (ETHUSDT, SOLUSDT). mark_prices has a real archive
partition for ETHUSDT/2026-05, so real ARCHIVE_ONLY/SQLITE_ONLY/HYBRID
smokes are proven for ETHUSDT directly against the real DB; SOLUSDT (no
mark_prices archive) resolves SQLITE_ONLY, proven via a synthetic
symbol-mismatch-safety fixture.

DELIBERATELY OUT OF SCOPE (asserted below to remain on direct SQL):
  - `build_liq_cache`: a `liquidations` range read -- out-of-allowlist
    table, no archive partition / reader support.
  - `_bucket_events`/`_evaluate_trade` (imported from
    `tools.s34_shadow_paper_runner`): external module calls that own
    their own direct-SQL internals in that module -- untouched here,
    same accepted pattern as the ASOF-track files that call into this
    same shadow-paper-runner module (e.g. research_s34_eth_preliq_control.py).
...
import tools.research_s34_sell_regime_analysis as mod
from tools.research_s34_sell_regime_analysis import _mark_prices_range, _mark_prices_range_v2
...
def test_build_liq_cache_left_on_direct_sql_out_of_allowlist():
    import inspect
    src = inspect.getsource(mod.build_liq_cache)
    assert "liquidations" in src
    assert "RR.execute_read(" not in src


def test_build_mark_cache_uses_v2_not_inline_query():
    import inspect
    src = inspect.getsource(mod.build_mark_cache)
...
# ---------------------------------------------------------------------------
# Synthetic fixtures: symbol-mismatch safety (SOLUSDT has no real mark_prices
# archive; confirm the helper stays symbol-generic and never leaks) +
...
def test_solusdt_real_call_site_resolves_sqlite_only_synthetic(tmp_path):
    # SOLUSDT has NO mark_prices archive partition in production -> real
    # usage of this file for SOLUSDT resolves SQLITE_ONLY. Proven here via
    # a synthetic root with no catalog entries (mirrors the real absence).
    root = str(tmp_path / "r")
    os.makedirs(root)
    _catalog(root, [])
    db = str(tmp_path / "s.sqlite")
...
def test_trust_failure_fails_closed_synthetic(tmp_path):
    ...
    with pytest.raises(RR.ArchiveTrustError):
        _mark_prices_range_v2(root, "ETHUSDT", MAY, MAY + 1_000)
