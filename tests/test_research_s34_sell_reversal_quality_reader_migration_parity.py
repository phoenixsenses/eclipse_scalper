"""Range-read consumer migration V3 parity #1 (BATCH-STORAGE-ROTATION-
RETENTION-RANGE-READ-CONSUMER-MIGRATION-V3).

`tools/research_s34_sell_reversal_quality.py`'s `sim_long` had an inline
...
DELIBERATELY OUT OF SCOPE (asserted below to remain on direct SQL):
  ...
  - `events`: a `liquidations` range read -- out-of-allowlist table, no
...
import tools.research_s34_sell_reversal_quality as mod
from tools.research_s34_sell_reversal_quality import _horizon_marks, _horizon_marks_v2
...
def test_inclusive_end_boundary_parity_real():
    ...
    assert at[-1][0] == last_ts       # the boundary row itself is included
...
def test_sim_long_uses_v2_not_inline_query():
    import inspect
    src = inspect.getsource(mod.sim_long)
...
def test_symbol_mismatch_safety_synthetic(tmp_path):
    ...
    _sqlite(db, [
        ...
        (3, MAY + 500, "SOLUSDT", 99999.0, 0.0, None),  # must not leak into hardcoded ETHUSDT read
    ])
...
def test_tie_break_ordering_parity_synthetic(tmp_path):
    ...
    _sqlite(db, [
        (1, MAY, "ETHUSDT", 3000.0, 0.0, None),
        (2, MAY, "ETHUSDT", 3000.5, 0.0, None),  # same ts_ms, tie-break by id ASC
    ])
    rows = _horizon_marks_v2(root, MAY, MAY, source_db_path=db)
    assert rows == [(MAY, 3000.0), (MAY, 3000.5)]  # id-ascending order preserved
