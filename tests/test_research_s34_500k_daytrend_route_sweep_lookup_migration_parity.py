"""ASOF Batch 4 consumer migration parity #3 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V7).

`tools/research_s34_500k_daytrend_route_sweep.py`'s `book_ticker_at` (an
...
`SYMBOL="ETHUSDT"` in both the oracle SQL and `book_ticker_at_v2` (this
file never varies it -- confirmed by direct source inspection). book_ticker's
only archived partition is SOLUSDT/2026-04 -- ETHUSDT has NO archive
coverage, so real production use resolves SQLITE_ONLY only. Archive-only
and hybrid coverage are proven with synthetic fixtures.

This file has two OTHER book_ticker/mark_prices queries left on direct SQL,
deliberately out of scope for this gate:
  - `mark_at`: a forward "at-or-after" lookup (ORDER BY ts_ms ASC LIMIT 1,
    ts_ms>=?) -- the opposite direction of `lookup_latest_at_or_before`'s
    as-of semantics, not covered by this helper at all.
  - `path_marks`: a bounded range read (ts_ms BETWEEN) on mark_prices --
    belongs to the range-read track, not this ASOF gate.
"""
...
from tools.research_s34_500k_daytrend_route_sweep import book_ticker_at, book_ticker_at_v2
...
def test_mark_at_left_on_direct_sql_out_of_scope():
    import inspect
    src = inspect.getsource(mod.mark_at)
    assert "mark_prices" in src and "order by ts_ms asc" in src
    assert "RR.lookup_latest_at_or_before(" not in src  # no call to the helper -- comment mentions it by name only
...
def test_exact_hit_and_between_and_staleness_synthetic(tmp_path):
    ...
    stale = book_ticker_at_v2(root, MAY + 2000 + mod.MAX_BOOK_STALENESS_SEC * 1000 + 1, source_db_path=db)
