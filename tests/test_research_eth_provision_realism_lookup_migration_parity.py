"""ASOF Batch 3 consumer migration parity #2 (BATCH-STORAGE-ROTATION-
RETENTION-ASOF-LOOKUP-CONSUMER-MIGRATION-V6).

`tools/research_eth_provision_realism.py`'s `book_at_or_before` (an
`ORDER BY ts_ms DESC LIMIT 1` as-of point lookup returning
`{ts_ms,bid,ask,mid}` via `book_row`) is migrated to `book_at_or_before_v2`
(reader-backed). `book_at_or_before` is kept unchanged as the parity oracle.

Call-site audit: the lookup's symbol is HARDCODED to the string literal
`'ETHUSDT'` in both the oracle SQL and `book_at_or_before_v2` (this file is
ETH-only by construction; confirmed by direct source inspection).
book_ticker's only archived partition is SOLUSDT/2026-04 -- ETHUSDT has NO
archive coverage, so real production use resolves SQLITE_ONLY only.
Archive-only and hybrid coverage are proven with synthetic fixtures.

The file's other book_ticker query, `first_cross` (a forward-scanning
bounded range read with a price predicate, ORDER BY ts_ms ASC LIMIT 1), is
deliberately left on direct SQL -- out of scope for the as-of point-lookup
track -- and is not exercised here.
"""
...
import tools.research_eth_provision_realism as mod
from tools.research_eth_provision_realism import book_at_or_before, book_at_or_before_v2
...
COLS = ("ts_ms", "bid_price", "ask_price", "mid_price")
...
def test_v2_parity_ethusdt_sqlite_only_exact_hit():
    ...
    assert new == old  # {ts_ms,bid,ask,mid} value + selected-column parity
...
def test_first_cross_left_on_direct_sql_out_of_scope():
    import inspect
    src = inspect.getsource(mod.first_cross)
...
def test_archive_only_lookup_synthetic(tmp_path):
    ...
    lk = RR.lookup_latest_at_or_before(root, table="book_ticker", symbol="ETHUSDT", ts_ms=APR + 5000, columns=COLS)
...
def test_hybrid_fallback_synthetic(tmp_path):
    ...
    _sqlite(db, [])
    q = MAY + 500
    lk = RR.lookup_latest_at_or_before(root, table="book_ticker", symbol="ETHUSDT", ts_ms=q, columns=COLS,
                                       source_db_path=db)
...
def test_symbol_mismatch_safety_synthetic(tmp_path):
    ...
    sol = RR.lookup_latest_at_or_before(root, table="book_ticker", symbol="SOLUSDT", ts_ms=MAY,
                                        columns=COLS, source_db_path=db)
...
def test_provenance_correctness_synthetic(tmp_path):
    ...
    db = str(tmp_path / "s.sqlite")
    _sqlite(db, _rows(1, MAY, 5))
    lk = RR.lookup_latest_at_or_before(root, table="book_ticker", symbol="ETHUSDT", ts_ms=MAY + 2000, columns=COLS,
                                       source_db_path=db)
